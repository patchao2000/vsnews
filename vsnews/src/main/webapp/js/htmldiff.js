...
//            return "<br/>[耗时："+(new Date - sd)+"毫秒]"+
        ...
        insert: function(opt) {
            this.insertTag('ins', 'background-color:#CFC', this.newWords.slice(opt.startInNew, opt.endInNew));
        },

        del: function(opt) {
            this.insertTag('del', 'color:#999; background-color:#FEC8C8', this.oldWords.slice(opt.startInOld, opt.endInOld));
        },
        ...
        warpText: function(text, tagName, cssCls) {
            return '<' + tagName + ' style="' + cssCls + '">' + text + '</' + tagName + '>';
